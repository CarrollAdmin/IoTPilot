...
def single_agent(process_id, model, question, max_attempts, program_name, role):
...
def main():
    ...
    for _, code_name in enumerate(code_names):
        ...
        for process_id in range(1,6):
            thread = threading.Thread(target=single_agent, args=(process_id, model, data_dict[code_name], max_attempts, code_name, role))
